...
def verify_chain():
    ...
    for (index, block) in enumerate(blockchain): # enumerate - index and value
        if index == 0:
           continue
...
while waiting_for_input:
    ...
    if user_choice == '1':
        ...
        if len(blockchain) >= 1:
            ...
            'previous_hash': '', 
            'index': 0,
            'transactions': [{'sender': 'SJ', 'recipient': 'DJ', 'amount': 102.0}]
        }
    elif user_choice == 'q':
    ...
    if not verify_chain():
        ...
        break
    print('Balance of {}: {:6.2f}'.format(owner, get_balance('Shubham')))
else:
    print('User left!')
